...
pub enum IserverMarketdataHistoryGetError {
    Status429(models::TrsrvFuturesGet500Response),
    Status500(models::SystemError),
    UnknownValue(serde_json::Value),
}
...
pub enum IserverMarketdataSnapshotGetError {
    Status400(models::IserverMarketdataSnapshotGet400Response),
    UnknownValue(serde_json::Value),
}
...
/// struct for typed errors of method [`md_snapshot_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum MdSnapshotGetError {
    Status500(),
    Status401(),
    UnknownValue(serde_json::Value),
}
...
pub async fn hmds_history_get(configuration: &configuration::Configuration, conid: i32, period: &str, bar: Option<&str>, outside_rth: Option<bool>) -> Result<models::HistoryResult, Error<HmdsHistoryGetError>> {
...
pub async fn iserver_marketdata_conid_unsubscribe_get(configuration: &configuration::Configuration, conid: &str) -> Result<models::IserverMarketdataConidUnsubscribeGet200Response, Error<IserverMarketdataConidUnsubscribeGetError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/iserver/marketdata/{conid}/unsubscribe", local_var_configuration.base_path, conid=crate::apis::urlencode(conid));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
...
pub async fn iserver_marketdata_history_get(configuration: &configuration::Configuration, conid: &str, period: &str, exchange: Option<&str>, bar: Option<&str>, outside_rth: Option<bool>) -> Result<models::HistoryData, Error<IserverMarketdataHistoryGetError>> {
    let local_var_configuration = configuration;
...
pub async fn iserver_marketdata_snapshot_get(configuration: &configuration::Configuration, conids: &str, since: Option<i32>, fields: Option<&str>) -> Result<Vec<models::IserverMarketdataSnapshotGet200ResponseInner>, Error<IserverMarketdataSnapshotGetError>> {
    let local_var_configuration = configuration;
...
pub async fn md_snapshot_get(configuration: &configuration::Configuration, conids: &str, fields: Option<&str>) -> Result<models::MarketData, Error<MdSnapshotGetError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/md/snapshot", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<MdSnapshotGetError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
